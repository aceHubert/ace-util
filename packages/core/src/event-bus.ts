  ...
  return {
    ...
    off(eventName, handler) {
      if (!handler || !isFunction(handler)) {
        warn(process.env.NODE_ENV === 'production', 'handler must be a valid event function');
        return;
      }
      if (!hasOwn(events, eventName)) {
        warn(process.env.NODE_ENV === 'production', `no event name of "${eventName}" is on listening`);
        return;
      }
